...
void create_pipe(const char npipe_path[NAMED_PIPE_PATH_LIM]) {
...
int open_pipe(const char npipe_path[NAMED_PIPE_PATH_LIM]) {
    int fd = open(npipe_path, O_WRONLY);
    ALWAYS_ASSERT(fd != -1, "Failed to open client named pipe")
    return fd;
}

int pipe_send(int op, const char *msg) {
    
} 

char *pipe_recv(int op) {

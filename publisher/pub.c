...
int main(int argc, char **argv) {
    ...
    int reg_pipe_fd = open(register_pipe_name, O_WRONLY);
    ALWAYS_ASSERT(reg_pipe_fd != -1, "Failed to open register FIFO!");

    send_proto_string(reg_pipe_fd, REGISTER_PUBLISHER, request);
    ALWAYS_ASSERT(close(reg_pipe_fd) == 0, "Failed to close register pipe!");
